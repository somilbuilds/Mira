...
export async function upsertEntry(userId, entryId, embedding, metadata) {
  ...
  const dreamDataString =
    typeof metadata.dreamData === 'string'
      ? metadata.dreamData
      : metadata.dreamData
        ? JSON.stringify(metadata.dreamData)
        : ''
  ...
  await ns.upsert([
    {
      ...
      metadata: {
        ...
        reflection: metadata.reflection?.substring(0, 1000) || metadata.summary?.substring(0, 1000) || '',
        summary: metadata.summary?.substring(0, 1000) || '',
        sentiments: metadata.sentiments || '',
        mood: metadata.mood || '',
        timestamp: metadata.timestamp || new Date().toISOString(),
        commitment: metadata.commitment || '',
        pattern: metadata.pattern || '',
        dreamData: dreamDataString.substring(0, 2500),
        dreamDate: metadata.dreamDate || '',
        dreamTime: metadata.dreamTime || '',
        sleepQuality: `${metadata.sleepQuality || ''}`,
        notesToSelf: metadata.notesToSelf?.substring(0, 500) || '',
        type: 'entry',
      },
    },
  ])
